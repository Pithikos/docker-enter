...
/*
 * Parses the command given by the user (uses optget)
 * 
	...
	while ( ns->name != NULL ) {
		if (ns->name == ns_name)
		{
			...
			if ((fd = open(pathbuf, O_RDONLY)) == -1) {
				perror("Trying to open namespace");
				exit(1);
			}
			
			if (setns(fd, ns->type) < 0) {
				perror("Trying to set namespace");
				exit(1);
			}
			return 0;
		}
